...
function CrudDetails(props) {
	...
	useEffect(
		function () {
			async function getCrudById() {
				try {
					const response = await axios.get(`http://3.90.202.141:8080/api/cruds/${_id}`);
	...
	async function handleDelete() {
		try {
			await axios.delete(`http://your-node-public-ip:8080http://3.90.202.141:8080/api/cruds/${_id}`);
			navigate("/cruds");
		} catch (error) {
			console.error(error);
		}
